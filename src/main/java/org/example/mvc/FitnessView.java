    ...
    private void addActivity(long userCode, boolean toTrainingPlan) {
        ...
        Consumer<Integer> handler =
                i -> {
                    ...
                    int minutes =
                            input.readInt(
                                    "Duration (min) > ", "Must be a positive integer!", m -> m > 0);
                    LocalDateTime date =
                            input.readDate(
                                    !toTrainingPlan,
                    ...
                    if (fields.contains(FitnessController.ActivityExtraField.WEIGHT)) {
                        values.put(
                                FitnessController.ActivityExtraField.WEIGHT,
                                input.readDouble(
                                        "Weight (kg) > ",
                    ...
                    if (fields.contains(FitnessController.ActivityExtraField.DISTANCE)) {
                        values.put(
                                FitnessController.ActivityExtraField.DISTANCE,
                                input.readDouble(
                                        "Distance (km) > ",
                                        "Must be a positive decimal!",
                                        d -> d > 0.0));
                    ...
                    if (fields.contains(FitnessController.ActivityExtraField.ALTIMETRY)) {
                        values.put(
                                FitnessController.ActivityExtraField.ALTIMETRY,
                                input.readDouble(
                                        "Altimetry > ",
                        ...
                        if (toTrainingPlan) {
                            int repetitions =
                                    input.readInt(
                                            ...
                                            "Must be a positive integer!",
                                            r -> r > 0);
                            this.controller.addActivityToTrainingPlan(
                                    userCode, classes[i], minutes, date, values, repetitions);
                        } else {
                            this.controller.addActivity(
                                    userCode, classes[i], minutes, date, values);
                        }
            ...
            new MenuEntry(
                    ...
                    i -> {
                        ...
                        MenuEntry[] activityEntries = {
                            ...
                            new MenuEntry("Add to training plan", j -> addActivity(code, true)),
    ...
    private void runQuery() {
        ...
        Consumer<Integer> handler =
                i -> {
                    ...
                    if (classes[i].equals("QueryDistance")) {
                        ...
                        MenuEntry[] queryEntries = {
                            new MenuEntry(
                                    "All distance",
                                    j -> {
                                        onlyAltimetry[0] = false;
                                    }),
                            new MenuEntry(
                                    "Altimetry only",
                                    j -> {
                                        onlyAltimetry[0] = true;
                                    }),
                        };
                    ...
                    try {
                        System.out.println(
                                this.controller.runQuery(
                                        classes[i], start, end, code, onlyAltimetry[0]));
                    } catch (FitnessControllerException e) {
                        System.err.println(e.getMessage());
                    }
                };
